...
const STUDIES: CaseStudy[] = [
  ...
  },
  {
    ...
    before: { tag: 'Olympic medals, prior 20 yrs', value: '0 since 2004' },
    after: { tag: 'Paris 2024', value: 'Silver 🥈' },
    startingPoint: {
      h: 'The margin was 0.66 points.',
    ...
    takeaway: {
      ...
      body: 'At that level, the difference between heartbreak and a podium isn’t physical — it’s the quality of decisions made when the pressure is highest. One year later, the team stood on the Olympic podium. The brain can be trained to perform differently under pressure.',
    },
  },
  {
...
function CaseStudyRow({ study, defaultOpen }: { study: CaseStudy; defaultOpen: boolean }) {
        ...
        <div className="overflow-hidden">
          <div className="px-6 sm:px-8 pb-8 pt-2 border-t border-line">
            ...
            <div className="mt-8 border-l-2 border-venom pl-5 max-w-3xl">
              ...
              <h4 className="text-bone text-xl font-extrabold uppercase tracking-tight mb-4">{study.takeaway.h}</h4>
              <p className="text-bone-dim text-[15px] leading-relaxed">{study.takeaway.body}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <section className="py-16 md:py-20 lg:py-[100px] bg-surface border-t border-line">
      <div className="max-w-[1200px] mx-auto px-5 sm:px-8">
        <div className="flex flex-col gap-5">
          {STUDIES.map((study, i) => (
            <CaseStudyRow key={study.label} study={study} defaultOpen={i === 0} />
          ))}
        </div>
      </div>
    </section>
